...
window.navisensMapsTouching = false;
window.navisensMapsTouchstart = null; // L.point containerPoint
var dragging = false;
...
function handleCustomRotation(e) {
    if (!customRotationHandle.offset) {
        ...
        if (map.latLngToContainerPoint(customRotationHandle.getLatLng()).distanceTo(rotPoint) <= 3)
            return;
        if (!window.navisensMapsTouchstart) return;
        customRotationHandle.offset = window.navisensMapsTouchstart.subtract(rotPoint);
...
move = function(id, x, y, heading, category) {
    ...
    if (id == userID) {
        ...
        if (!window.navisensMapsTouching && GET('shouldPan'))
...
function UI() {
    ...
    map.getContainer().addEventListener("touchstart", function (e) { window.navisensMapsTouching = true; window.navisensMapsTouchstart = L.point([e.touches[0].clientX, e.touches[0].clientY]); });
    map.getContainer().addEventListener("touchend", function (e) { window.setTimeout(function () { window.navisensMapsTouching = false; }, 0); });
