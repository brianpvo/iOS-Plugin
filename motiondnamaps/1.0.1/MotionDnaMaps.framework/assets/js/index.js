...
function lookAtRotation() {
    var vec = map.latLngToContainerPoint(customHeading.getLatLng()).subtract(map.latLngToContainerPoint(user.getLatLng()));
    customRotationIcon.heading = Math.atan2(vec.y, vec.x) * 180 / Math.PI + 90;
    customRotation.setIcon(customRotationIcon);

    customHeadingVector.setLatLngs([user.getLatLng(), customHeading.getLatLng()]);
}

function recalculateRotation(e) {
    var vec = map.latLngToContainerPoint(e.latlng).subtract(map.latLngToContainerPoint(user.getLatLng()));
    customRotationIcon.heading = Math.atan2(vec.y, vec.x) * 180 / Math.PI + 90;
    customRotation.setIcon(customRotationIcon);

    customHeading.setLatLng(e.latlng);
    customHeading.addTo(map);
    customHeadingVector.setLatLngs([user.getLatLng(), e.latlng]);
}
...
function handleCustomRotation(e) {
    ...
    customHeadingVector.setLatLngs([user.getLatLng(), user.getLatLng()]);
    customRotation.setIcon(customRotationIcon);
}
...
function recalculateCustomRotation() {
    ...
    customRotationHandle.setLatLng(user.getLatLng());
...
function onMapLongTouch(x, y) {
    onClickLocation(toggle);
    if (customLocation != null) {
        customLocation.setLatLng(map.containerPointToLatLng([x, y]));
...
function onClickLocation(control) {
    ...
    customLocation = L.marker(user.getLatLng(), {draggable: true, keyboard: false, opacity: 0.8, zIndexOffset: 999});
    ...
    confirm = function() {
        ...
        volatileDistance = volatileLocation.distanceTo(user.getLatLng()) / 2;
        if (volatileDistance < MIN_DISTANCE)
            volatileDistance = MIN_DISTANCE;
        volatileSamples = 0;

        webkit.messageHandlers.customLocationInitialized.postMessage([customLocation.getLatLng().lat, customLocation.getLatLng().lng, userIcon.heading]);
...
function onClickHeading(control) {
    ...
    var location = (customLocation || user).getLatLng();

    customRotation = L.marker(location, {keyboard: false, icon: customRotationIcon, opacity: 0.75, zIndexOffset: 99});
    customRotationIcon.heading = userIcon.heading;
    customRotation.addTo(map);

    var headingLocation = map.latLngToContainerPoint(location);
    var offset = L.point(Math.sin(userIcon.heading * Math.PI / 180), -Math.cos(userIcon.heading * Math.PI / 180)).multiplyBy(100);
    ...
    customHeadingVector = L.polyline([user.getLatLng(), user.getLatLng()], {color: '#555', opacity: 0.8, weight: 1.5}).addTo(map);
    ...
    confirm = function() {
        webkit.messageHandlers.customLocationInitialized.postMessage([user.getLatLng().lat, user.getLatLng().lng, customRotationIcon.heading]);
        clean();
    };
...
addPoint = function(x, y, category) {
    if (volatileLocation) {
...
function RUN(needsGPS) {
    ...
    map.getContainer().addEventListener("touchstart", () => { window.setTimeout( () => { touching = true; }, 0); });
    map.getContainer().addEventListener("touchend", () => { window.setTimeout( () => { touching = false; }, 0); });

    // ===== INTERFACE =====

    user = L.marker(GET('usercoords'));
...
function UI() {
    ...
    map.getContainer().addEventListener("touchstart", function (e) {
        var l = e.touches.length - 1;
        touchstart = L.point([e.touches[l].clientX, e.touches[l].clientY]);
        if (touching) {
            if (longtouch) {
            ...
            longtouch = setTimeout( () => {
               onMapLongTouch(e.touches[l].clientX, e.touches[l].clientY);
               longtouch = null;
            }, 750 );
        }
    });
    map.getContainer().addEventListener("touchmove", function (e) {
        var l = e.touches.length - 1;
        if ((Math.abs(e.touches[l].clientX - touchstart.x) > 15/2 ||
                Math.abs(e.touches[l].clientY - touchstart.y) > 15/2) &&
                longtouch) {
            clearTimeout(longtouch);
            longtouch = null;
        }
